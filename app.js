...
let _feedOldestTs=null;      // timestamp of oldest loaded post (for next page)
let _feedLoading=false;      // guard against double-loads
let _feedExhausted=false;    // no more posts to load
let _feedScrollHandler=null; // so we can remove it on teardown
...
async function renderFeed(){
  ...
  // Real-time listener ONLY for brand-new posts (limitToLast 1 watches tip of list)
  _feedUnsubscribe=window.XF.on('posts',async function(snap){
    // Only care about genuinely new posts appearing AFTER initial load
    if(!_feedOldestTs)return;
    // Find any post newer than what we already have at the top
    const topPost=container.querySelector('.post[data-id]');
    if(!topPost)return;
    // Get current newest timestamp from DOM
    let newestTs=0;
...
async function _loadFeedPage(container,isFirst){
  ...
  try{
    const blockedUids=await getBlockedUids();
    // Query: order by createdAt, newest first = limitToLast from endAt
    ...
    // If we got fewer than PAGE_SIZE+1, no more pages
    ...
    // Insert before sentinel (end marker)
...
function _attachFeedScrollListener(container){
  // Works for both mobile (window scroll) and desktop (main-content scroll)
  const mc=document.querySelector('.main-content');
  _feedScrollHandler=function(){
    ...
    if(rect.top<threshold)_loadFeedPage(container,false);
  };
...
async function acceptConnectionFromProfile(reqId,fromUid,btn){
  ...
  await acceptConnection(reqId,fromUid);
  // Re-render profile so buttons update to Connected + Message
...
async function renderUserProfile(uid){
  ...
  try{
    ...
    let connStatus='none';
    let incomingReqId=null; // reqId if THEY sent a request to ME
    if(currentUser){
      const cs=await window.XF.get('connections/'+currentUser.uid+'/'+uid);
      if(cs.exists()){
        connStatus='connected';
      }else{
        // Check: did I send to them?
        const rs1=await window.XF.get('connectionRequests/'+currentUser.uid+'_'+uid);
        // Check: did they send to me?
        const rs2=await window.XF.get('connectionRequests/'+uid+'_'+currentUser.uid);
        if(rs1.exists()&&rs1.val().status==='pending'){
          connStatus='pending'; // I sent, waiting
        }else if(rs2.exists()&&rs2.val().status==='pending'){
          connStatus='incoming'; // They sent to me — I can accept here
          incomingReqId=uid+'_'+currentUser.uid;
        }
      }
    }
    const postsSnap=await window.XF.get('posts');const posts=[];
...
function startNotifWatch(){
  ...
  window.XF.on('notifications/'+currentUser.uid,snap=>{
    ...
    // Deduplicate connection_request per sender (same logic as renderNotifications)
    const seenConnReq=new Set();
    let unread=0;
    snap.forEach(c=>{
      const n=c.val();
    ...
    // If notifications page is currently open, re-render it so new notifs appear instantly
    if(activePage==='notifications')renderNotifications();
  });
}
async function startMsgWatch(){
  if(!currentUser)return;
  // Watch connections in real-time, then watch each DM conv
  window.XF.on('connections/'+currentUser.uid,async function(connSnap){
    ...
    // Re-count unread across all convs whenever any connection changes
    refreshMsgBadge();
  });
  // Also do an immediate count on login
  refreshMsgBadge();
}
...
async function renderConversations(){
  ...
  try{
    ...
    await Promise.all(uids.map(async uid=>{
      ...
      // Only fetch last 50 messages — enough for preview + unread count, no need for full history
...
async function openDMWith(uid){
  ...
  // Live message listener — limited to last 100 messages
  ...
  msgUnsubscribe=(function(){
    ...
    const wasAtBottom=msgEl.scrollHeight-msgEl.scrollTop-msgEl.clientHeight<80;

    // Render all messages — images use lazy loading (load on demand)
    msgEl.innerHTML=msgs.map(function(m){
      const isMe=m.senderUid===currentUser.uid;
      let imgHTML='';
      if(m.imageUrl){
        // Image is loaded only when this chat is open — use native lazy loading
        imgHTML=`<img src="${escapeHTML(m.imageUrl)}" loading="lazy" onclick="openLightbox('${escapeHTML(m.imageUrl)}')" alt="photo" style="max-width:220px;border-radius:12px;display:block;cursor:pointer">`;
      }
    ...
    // Mark as read after a short delay — outside the render so it doesn't loop
...
async function markMessagesRead(convId){
  ...
  try{
    ...
    if(Object.keys(updates).length>0){
      ...
      // Immediately refresh badge after marking read
      refreshMsgBadge();
    }
  }catch(e){}
}
