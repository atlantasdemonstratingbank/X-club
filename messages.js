...
async function renderConversations() {
  ...
  try {
    ...
    // Fetch last 50 messages per conversation (no orderByChild — just limitToLast)
    const previews = {}, unreadCounts = {}, lastTimes = {};
    await Promise.all(uids.map(async uid => {
      const convId = [currentUser.uid, uid].sort().join('_');
      const dmSnap = await window.XF.db.ref('dms/' + convId).limitToLast(50).once('value');
      if (dmSnap.exists()) {
        const msgs = [];
        dmSnap.forEach(c => msgs.push({ id: c.key, ...c.val() }));
        // Sort by createdAt ascending so last message is msgs[msgs.length-1]
        msgs.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
        previews[uid] = msgs[msgs.length - 1];
        lastTimes[uid] = msgs[msgs.length - 1]?.createdAt || 0;
